...
<?php
	...
	if($commentNewCount >= (count($arr) - 1)){
		echo '<div>';
		echo '<a href="Animeupdatelive.php">RESULT</a>';
		echo '<br>';
	 ...
	 //image putting into array;
  	...
  	foreach ($arr as $ar) {
  		if($src == '0'){
			...
			//echo "doing";
			if($src == "https://personalthing.000webhostapp.com/animes/".$ar['image']){
				$t = $ar['image'];
				//echo $t;
				$s =  "UPDATE anime SET score=score+50 WHERE image='$t'";
				$a = mysqli_query($db,$s);
				if($a){
					echo "done";
  	...
  	if($check == 1){
		return 0;
    		...
    		echo '<img src="animes/'.$arr[$commentNewCount]['image'].'" id="img"/>';
    		echo "_____________";
    		echo '<img src="animes/'.$arr[$commentNewCount+1]['image'].'" id="img1"/>';
    		echo '<br>';
    		echo '<br>';
    		echo '<br>';
